...
pub fn initialize_pool_instr(
    ...
    let (amm_config_key, __bump) = Pubkey::find_program_address(&[AMM_CONFIG_SEED.as_bytes(), &amm_config_index.to_be_bytes()], &program.id());
    ...
    let (lp_mint_key, __bump) = Pubkey::find_program_address(&[POOL_LP_MINT_SEED.as_bytes(), pool_account_key.to_bytes().as_ref()], &program.id());
    ...
    let instructions = program
        ...
        .accounts(raydium_cp_accounts::Initialize {
            ...
            creator_lp_token: spl_associated_token_account::get_associated_token_address(&program.payer(), &lp_mint_key),
            token_0_vault,
            token_1_vault,
            create_pool_fee,
